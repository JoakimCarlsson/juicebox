...
func (t *ScanMemoryTool) Info() tool.ToolInfo {
	return tool.NewToolInfo(
		"scan_memory",
		"Search the live process heap for a byte pattern. Finds secrets like JWT tokens (search 'eyJ'), API keys (search 'sk_live_', 'AKIA'), passwords, Bearer tokens, and any other strings or byte sequences in memory. Returns matching addresses with hex dump and ASCII preview of surrounding bytes.",
		ScanMemoryParams{},
	)
}
