...
func NewFetchWebpage() *FetchWebpageTool {
	return &FetchWebpageTool{}
}

func (t *FetchWebpageTool) Info() tool.ToolInfo {
	return tool.NewToolInfo(
		"fetch_webpage",
		"Fetch a URL and return the page content as Markdown. Use to read CVE pages, SDK documentation, vendor security advisories, or any web page relevant to the analysis.",
		FetchWebpageParams{},
	)
}

func (t *FetchWebpageTool) Run(
	ctx context.Context,
	params tool.ToolCall,
) (tool.ToolResponse, error) {
	input, err := agent.ParseToolInput[FetchWebpageParams](params.Input)
	if err != nil {
	...
	if err != nil {
		return tool.NewTextErrorResponse(
			fmt.Sprintf("read body failed: %v", err),
		), nil
	}
	...
	content, err := htmltomarkdown.ConvertString(html)
	if err != nil {
		return tool.NewTextErrorResponse(
