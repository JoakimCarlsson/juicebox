...
func BuildEditError(
	...
	fmt.Fprintf(
		&sb,
		"# %d SEARCH/REPLACE %s failed to match!\n",
		len(results.Failed),
		blocks,
	)
	...
	for _, fail := range results.Failed {
		...
		fmt.Fprintf(
			&sb,
			"<<<<<<< SEARCH\n%s=======\n%s>>>>>>> REPLACE\n\n",
			ensureNewline(fail.Block.Search),
			ensureNewline(fail.Block.Replace),
		)
		...
		if exists {
			...
			if similar != "" {
				fmt.Fprintf(
					&sb,
					"Did you mean to match some of these actual lines from %s?\n\n```\n%s\n```\n\n",
					fail.Block.Filename,
					similar,
				)
			}

			if fail.Block.Replace != "" &&
				strings.Contains(
					content,
					strings.TrimSpace(fail.Block.Replace),
				) {
				fmt.Fprintf(
					&sb,
	...
	sb.WriteString(
		"The SEARCH section must exactly match an existing block of lines including all white space, comments, indentation, etc\n",
	)
