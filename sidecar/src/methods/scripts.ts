...
export async function handleRunScript(
  ...
  try {
    ...
    userScript.message.connect(
      (_message: frida.Message, _data: Buffer | null) => {
        const msg = _message as { type: string; payload?: unknown };
        if (msg.type === "send" && msg.payload != null) {
          scriptState.messages.push(msg.payload);

          const line =
            JSON.stringify({ type: "script_output", payload: msg.payload }) +
            "\n";
          ...
          if (
            ...
            (msg.payload as Record<string, unknown>).__done === true
          ) {
            scriptState.done = true;
            resolveDone?.();
          }
        } else if (msg.type === "error") {
          const description = (msg as Record<string, unknown>).description ??
            String(msg);
...
export async function handleGetScriptOutput(
  req: JsonRpcRequest,
): Promise<JsonRpcResponse> {
  const sessionId = req.params?.sessionId as string;
  ...
  if (!scriptState) {
    return fail(req.id, -32602, `no running script named "${name}"`);
  }
...
export async function handleStopScript(
  req: JsonRpcRequest,
): Promise<JsonRpcResponse> {
  const sessionId = req.params?.sessionId as string;
  const name = req.params?.name as string;
  ...
  try {
    await scriptState.script.unload();
  } catch {}
  state.userScripts.delete(name);
