...
function RequestPhaseEditor({
  ...
  const [headers, setHeaders] = useState<[string, string][]>(() => Object.entries(pending.headers))
  ...
  const hasChanges =
    ...
    url !== pending.url ||
    body !== (pending.body ?? '') ||
    JSON.stringify(Object.fromEntries(headers)) !== JSON.stringify(pending.headers)
  ...
  const updateHeader = useCallback((index: number, field: 0 | 1, value: string) => {
    setHeaders((prev) =>
      prev.map((entry, i) => {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
        ...
        <span className="text-xs text-muted-foreground font-mono truncate">{pending.id}</span>
      </div>
...
function ResponsePhaseEditor({
  ...
  const [statusCode, setStatusCode] = useState(String(pending.statusCode ?? 200))
  ...
  const hasChanges =
    ...
    JSON.stringify(Object.fromEntries(headers)) !== JSON.stringify(pending.responseHeaders ?? {})
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-auto px-4 py-3 space-y-4">
        <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground">
          <Badge
            variant="secondary"
            className={cn('font-mono text-[10px] px-1.5 py-0', methodColor(pending.method))}
          >
            {pending.method}
          </Badge>
        ...
        <div>
          <h4 className="text-xs font-medium text-muted-foreground mb-1.5">Status Code</h4>
          <Input
            value={statusCode}
        ...
        <div>
          <h4 className="text-xs font-medium text-muted-foreground mb-1.5">Response Body</h4>
          <textarea
            value={body}
            onChange={(e) => setBody(e.target.value)}
            className="w-full min-h-[120px] rounded border border-border bg-background px-3 py-2 text-xs font-mono resize-y outline-none focus:ring-1 focus:ring-ring"
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <Button variant="ghost" size="sm" className="h-6 px-2 text-[10px]" onClick={onAdd}>
...
function ActionBar({
  ...
  return (
    <div className="flex items-center gap-2 border-t border-border px-4 py-2.5">
      ...
      {hasChanges && (
        <Button variant="secondary" size="sm" className="h-8" onClick={onForwardOriginal}>
          <Undo2 className="mr-1.5 h-3 w-3" />
          Forward Original
        </Button>
      )}
      <Button variant="destructive" size="sm" className="h-8 ml-auto" onClick={onDrop}>
