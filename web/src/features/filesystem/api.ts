...
export async function listFiles(sessionId: string, path: string): Promise<ListResponse> {
  const res = await fetch(`/api/v1/sessions/${sessionId}/fs/ls?path=${encodeURIComponent(path)}`)
  if (!res.ok) {
...
export async function readFile(sessionId: string, path: string): Promise<FileContent> {
  const res = await fetch(`/api/v1/sessions/${sessionId}/fs/read?path=${encodeURIComponent(path)}`)
  if (!res.ok) {
    const data = await res.json().catch(() => ({}))
